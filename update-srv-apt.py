...
def update_upgrade_apt(host):
    ...
    ssh = subprocess.Popen(["ssh", "-o", "StrictHostKeyChecking=no", "%s" % "root@"+str(host), "apt-get update && apt-get upgrade -y && apt-get dist-upgrade -y && apt-get autoremove -y && apt-get autoclean && apt-get clean"],
                       shell=False,
