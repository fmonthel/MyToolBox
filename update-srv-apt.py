...
def update_upgrade_apt(host):
    ...
    ssh = subprocess.Popen(["ssh", "-o", "StrictHostKeyChecking=no", "%s" % "root@"+host, "apt-get update && apt-get upgrade -y && apt-get dist-upgrade -y && apt-get autoremove && apt-get autoclean && apt-get clean"],
                       shell=False,
